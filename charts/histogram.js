...
class Histogram extends Chart {
  ...
  /**
   * Create the data object for histogram chart
   * @param data
   * @returns {Array}
   */
  getData(data) {
    const resp = [];
    _.each(data, (d) => {
      const trace = new HistogramTrace(d);
  ...
  /**
   * Create layout object for histogram chart
   * @returns {{title: *, xaxis: ({title: string, showgrid: boolean}|*),
   * yaxis: ({title: string, showgrid: boolean}|*), showlegend: *, legend: *, barmode: string}}
   */
  getLayout() {
    return {
      title: this.title,
